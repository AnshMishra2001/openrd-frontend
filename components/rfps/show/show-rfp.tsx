...
import { getDisputes, getRFP, getUser } from "@/lib/indexer"
...
// import { ApplicationCreationForm } from "@/components/rfps/forms/application-creation-form"
...
export function ShowRFP({
  ...
  const getIndexerDisputes = async () => {
    const newDisputes = await getDisputes(chainId, rfpId)
        ...
        >
          ...
          <TabsContent value="general">
            <div className="grid grid-cols-1 space-y-3 text-xs md:text-base">
              ...
              >
                Task Manager: <span className="break-words hover:text-[#0354EC]">{tasksManagerTitle ?? "Unknown"}</span>
              </Link>
          ...
          <TabsContent value="submissions">
            <div className="space-y-7">
                ...
                {/* {objectKeysInt(submissions)
                  ...
                  .map((submissionId) => (
                    <ShowSubmission
                      ...
                      task={blockchainRFP ?? indexerRFP}
                      indexerMetadata={
                        indexerRFP?.submissions[submissionId]?.cachedMetadata
                      }
                      indexerFeedbackMetadata={
                        indexerRFP?.submissions[submissionId]?.cachedFeedback
                      }
                      refresh={refresh}
                    />
                  ))} */}
              {/* </div> */}
              {/* <Separator />
              {(blockchainRFP || indexerRFP) && state === TaskState.Taken && (
                <div className="space-y-5">
                  <p className="text-2xl">Create submission:</p>
                  <SubmissionCreationForm
                    chainId={chainId}
                    taskId={taskId}
                    task={(blockchainRFP ?? indexerRFP) as Task} // Cannot be undefined because of the conditional render
                    refresh={refresh}
                  />
                </div>
              )} */}
              ...
              <ul>
                {objectKeysInt(cancelTaskRequests).map((requestId, i) => (
                  <ShowCancelTaskRequest
                    ...
                    indexerMetadata={
                      indexerRFP?.cancelTaskRequests[requestId]?.cachedMetadata
                    }
                    task={blockchainRFP ?? indexerRFP}
                    refresh={refresh}
                  />
...
function compareTasks(
  ...
  // objectKeysInt(blockchainRFP.applications).forEach((applicationId) => {
  //   const applicationWarning = `${baseWarning}: application ${applicationId}`
  //   const blockchain = blockchainRFP.applications[applicationId]
  //   const indexed = indexerRFP.applications[applicationId]
  ...
  // })
  // if (
  //   Object.keys(indexerRFP.applications).length >
  //   Object.keys(blockchainRFP.applications).length
  // ) {
  ...
  // objectKeysInt(blockchainRFP.cancelTaskRequests).forEach(
  //   (cancelTaskRequestId) => {
  //     const cancelTaskRequestWarning = `${baseWarning}: cancelTaskRequest ${cancelTaskRequestId}`
  //     const blockchain = blockchainRFP.cancelTaskRequests[cancelTaskRequestId]
  //     const indexed = indexerRFP.cancelTaskRequests[cancelTaskRequestId]
  ...
  // )
  // if (
  //   Object.keys(indexerRFP.cancelTaskRequests).length >
  //   Object.keys(blockchainRFP.cancelTaskRequests).length
  // ) {
  //   console.warn(
  ...
  // objectKeysInt(blockchainRFP.submissions).forEach((submissionId) => {
  //   const submissionWarning = `${baseWarning}: submission ${submissionId}`
  //   const blockchain = blockchainRFP.submissions[submissionId]
  //   const indexed = indexerRFP.submissions[submissionId]
  ...
  // })
  // if (
  //   Object.keys(indexerRFP.submissions).length >
  //   Object.keys(blockchainRFP.submissions).length
  // ) {
